...
echo ""
echo "多人对战（跨机器必须做这一步）："
echo "  1) 在这台机器上另起一个 STUN：python3 ministun.py 0.0.0.0 3478"
echo "  2) 编辑 $WEBROOT/config.ini，把 lanStunUrl 填成 stun:<本机内网IP>:3478"
echo "  不配 STUN 时浏览器只上报 mDNS(*.local) 候选，同机可连、跨机器基本连不上。"
echo "  配好后：主菜单 → 局域网(LAN) → 创建房间，把邀请码给对方。"
